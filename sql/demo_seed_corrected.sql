-- ============================================================================  
-- OTT Demo Data Seed (Corrected for actual schema)
-- ============================================================================  
-- HAPPLYZ MEDICAL SAS
...
INSERT INTO device_configurations (device_id, firmware_version, ota_pending, configuration_json, created_at, updated_at) VALUES
  (1, 'v2.5.0', false, '{"measurement_interval": 300, "battery_threshold": 20, "apn": "free", "server_url": "https://ott-api.example.com"}', NOW(), NOW()),
  (2, 'v2.5.0', false, '{"measurement_interval": 600, "battery_threshold": 15, "apn": "orange", "server_url": "https://ott-api.example.com"}', NOW(), NOW()),
  (3, 'v2.5.0', true, '{"measurement_interval": 300, "battery_threshold": 25, "apn": "sfr", "server_url": "https://ott-api.example.com"}', NOW(), NOW()),
  (4, 'fw_ott_optimized', false, '{"measurement_interval": 300, "battery_threshold": 20, "apn": "bouygues", "server_url": "https://ott-api.example.com"}', NOW(), NOW()),
  (5, 'v2.5.0', false, '{"measurement_interval": 300, "battery_threshold": 30, "apn": "free", "server_url": "https://ott-api.example.com"}', NOW(), NOW())
...
INSERT INTO measurements (device_id, timestamp, flowrate, battery_level, signal_strength, temperature, created_at) VALUES
  (1, NOW() - INTERVAL '10 minutes', 2.5, 85, -65, 22.5, NOW()),
  (1, NOW() - INTERVAL '20 minutes', 2.3, 86, -67, 22.3, NOW()),
  (1, NOW() - INTERVAL '30 minutes', 2.7, 87, -64, 22.7, NOW()),
  (2, NOW() - INTERVAL '5 minutes', 1.8, 92, -58, 21.8, NOW()),
  (2, NOW() - INTERVAL '15 minutes', 2.1, 93, -56, 21.9, NOW()),
  (4, NOW() - INTERVAL '15 minutes', 3.2, 78, -72, 23.2, NOW()),
  (4, NOW() - INTERVAL '30 minutes', 2.9, 79, -70, 23.0, NOW())
ON CONFLICT DO NOTHING;

-- Alertes de démo (adapté au schéma réel)
INSERT INTO alerts (id, device_id, type, severity, message, status, created_at) VALUES
  ('alert_001', 3, 'offline', 'high', 'Dispositif hors ligne depuis plus de 2 heures', 'unresolved', NOW() - INTERVAL '2 hours'),
  ('alert_002', 5, 'battery', 'medium', 'Batterie faible (12%)', 'unresolved', NOW() - INTERVAL '30 minutes'),
  ('alert_003', 1, 'measurement', 'low', 'Absence de mesures depuis 10 minutes', 'resolved', NOW() - INTERVAL '1 hour')
ON CONFLICT (id) DO UPDATE SET 
  type = EXCLUDED.type,
