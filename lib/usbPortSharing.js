...
if (typeof window !== 'undefined') {
  try {
    ...
    // Fallback si logger n'est pas disponible - utiliser console directement (OK pour fallback)
    logger = {
      debug: (...args) => console.debug(...args),
      warn: (...args) => console.warn(...args),
      error: (...args) => console.error(...args),
      log: (...args) => console.log(...args) // Fallback acceptable
    }
  }
}
...
class UsbPortSharing {
  ...
  requestMaster() {
    // Vérifier d'abord si le master actuel est mort (heartbeat expiré)
    const currentState = this.getStorageState()
    if (currentState && currentState.masterTabId) {
      const timeSinceHeartbeat = Date.now() - (currentState.lastHeartbeat || 0)
      if (timeSinceHeartbeat > HEARTBEAT_TIMEOUT) {
        // Le master est mort, on peut devenir master immédiatement
        if (typeof logger !== 'undefined' && logger.debug) {
          logger.debug('[USB Sharing] requestMaster: Master timeout detected, becoming master')
        }
        this.becomeMaster()
        return Promise.resolve(true)
      }
    } else if (!currentState || !currentState.masterTabId) {
      // Pas de master, on peut devenir master immédiatement
      if (typeof logger !== 'undefined' && logger.debug) {
        logger.debug('[USB Sharing] requestMaster: No master, becoming master')
      }
      this.becomeMaster()
      return Promise.resolve(true)
    }
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const state = this.getStorageState()
        // Vérifier à nouveau le heartbeat après l'attente
        if (state && state.masterTabId) {
          const timeSinceHeartbeat = Date.now() - (state.lastHeartbeat || 0)
          if (timeSinceHeartbeat > HEARTBEAT_TIMEOUT) {
            // Le master est mort pendant l'attente, devenir master
            if (typeof logger !== 'undefined' && logger.debug) {
              logger.debug('[USB Sharing] requestMaster: Master died during wait, becoming master')
            }
            this.becomeMaster()
            resolve(true)
            return
  ...
  handleStorageChange(newValue) {
    try {
      ...
      if (typeof logger !== 'undefined' && logger.warn) {
        logger.warn('[USB Sharing] Error parsing storage change:', _e)
      }
    }
  }
  ...
  updateStorage(state) {
    ...
    try {
      ...
      if (typeof logger !== 'undefined' && logger.warn) {
        logger.warn('[USB Sharing] Error updating storage:', _e)
      }
    }
  }
  ...
  forceReset() {
    // Forcer la réinitialisation de l'état (utile si l'état est corrompu)
    if (typeof logger !== 'undefined' && logger.warn) {
      logger.warn('[USB Sharing] Force reset - clearing all state')
    }
    this.stopHeartbeat()
    localStorage.removeItem(STORAGE_KEY)
    this.isMaster = false
    ...
    // Redémarrer le heartbeat et devenir master
    this.startHeartbeat()
    this.becomeMaster()
  }
}
