...
/**
 * Construit le payload pour la commande UPDATE_CALIBRATION
 * @param {string|number} calA0 - Coefficient a0
 * @param {string|number} calA1 - Coefficient a1
 * @param {string|number} calA2 - Coefficient a2
 * @returns {Object} Payload pour UPDATE_CALIBRATION avec validation
 * @throws {Error} Si les coefficients sont invalides
 */
...
/**
 * Construit le payload pour la commande UPDATE_CALIBRATION depuis un tableau
 * @param {Array<number>} coefficients - Tableau [a0, a1, a2]
 * @returns {Object} Payload pour UPDATE_CALIBRATION
 * @throws {Error} Si les coefficients sont invalides
 */
export function buildUpdateCalibrationPayloadFromArray(coefficients) {
  if (!Array.isArray(coefficients) || coefficients.length < 3) {
    throw new Error('Les coefficients doivent être un tableau de 3 éléments [a0, a1, a2]')
  }
  
  return buildUpdateCalibrationPayload(coefficients[0], coefficients[1], coefficients[2])
}
...
/**
 * Crée une commande UPDATE_CALIBRATION pour un dispositif
 * @param {Function} fetchWithAuth - Fonction d'authentification
 * @param {string} API_URL - URL de l'API
 * @param {string} iccid - ICCID du dispositif
 * @param {string|number} calA0 - Coefficient a0
 ...
 * @returns {Promise<Object>} Réponse de l'API
 */
export async function createUpdateCalibrationCommand(fetchWithAuth, API_URL, iccid, calA0, calA1, calA2, options = {}) {
  const payload = buildUpdateCalibrationPayload(calA0, calA1, calA2)
  return await createOtaCommand(fetchWithAuth, API_URL, iccid, 'UPDATE_CALIBRATION', payload, options)
}
