...
/**
 * Construit une URL avec le basePath, en évitant les doubles slashes
 * @param {string} path - Chemin relatif (ex: '/dashboard')
 * @param {string} basePath - Base path (ex: '/OTT' ou '')
 * @returns {string} URL complète sans doubles slashes ni paramètres vides
 */
export function buildUrl(path = '', basePath = '') {
  // Normaliser le basePath (supprimer le slash final s'il existe)
  const normalizedBase = basePath.replace(/\/+$/, '')
  
  // Normaliser le path (commencer par /, supprimer les doubles slashes)
  ...
  // Éviter le double slash entre basePath et path
