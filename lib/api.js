...
export async function fetchJson(fetchWithAuth, API_URL, path, options = {}, config = {}) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[fetchJson] Erreur HTTP:', JSON.stringify(errorInfo, null, 2))
      console.error('[fetchJson] Erreur HTTP (objet):', errorInfo)
      ...
      if (response) {
        try {
          ...
          console.error('[fetchJson] Détails réponse:', {
            ok: response.ok,
          ...
          console.error('[fetchJson] Erreur lors de l\'extraction des détails réponse:', e)
        }
      } else {
        console.error('[fetchJson] ⚠️ response est null ou undefined')
      }
      
      // Toujours afficher le texte de la réponse
      console.error('[fetchJson] Texte de la réponse:', {
        hasText: !!text,
        textLength: text ? text.length : 0,
        textPreview: text ? text.substring(0, 500) : '(vide)',
        textFull: text || '(vide)',
      ...
      if (text && text.length > 0) {
        if (!isJson) {
          console.error('[fetchJson] Réponse non-JSON (premiers 1000 caractères):', text.substring(0, 1000))
        } else {
          console.error('[fetchJson] Réponse JSON (erreur, premiers 1000 caractères):', text.substring(0, 1000))
          try {
            const parsed = JSON.parse(text)
            console.error('[fetchJson] Réponse JSON parsée:', parsed)
          } catch (e) {
            console.error('[fetchJson] Impossible de parser le JSON:', e)
          }
        }
      } else {
        console.error('[fetchJson] ⚠️ Réponse vide (pas de texte)')
      }
