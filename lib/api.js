  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorInfo = {
        ...
        isJson: isJson || false,
        errorData: (errorData && Object.keys(errorData).length > 0) ? errorData : null,
      ...
      // Toujours logger, même si certaines valeurs sont manquantes
      console.error('[fetchJson] Erreur HTTP:', errorInfo)
      ...
      if (text && text.length > 0) {
        if (!isJson) {
          console.error('[fetchJson] Réponse non-JSON:', text.substring(0, 1000))
        } else {
          console.error('[fetchJson] Réponse JSON (erreur):', text.substring(0, 1000))
        }
      } else {
        console.error('[fetchJson] Réponse vide (pas de texte)')
      }
