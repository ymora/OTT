...
export async function enqueueMeasurement(measurementData) {
  try {
    ...
    logger.error('Erreur ajout mesure à la queue:', err)
    return false
  }
}
...
export async function getPendingMeasurements(limit = 50) {
  try {
    ...
    logger.error('Erreur récupération mesures:', err)
    return []
  }
}
...
export async function incrementRetry(id) {
  try {
    ...
    logger.error('Erreur incrément retry:', err)
    return null
  }
}
...
export async function cleanupOldMeasurements() {
  try {
    ...
    logger.error('Erreur nettoyage mesures:', err)
    return 0
  }
}
