...
2. Vous devriez voir : `✅ Core ESP32 déjà installé dans /opt/render/project/src/arduino-data`
...
- **Persistance** : Les données dans `/opt/render/project/src/arduino-data` sont conservées entre les déploiements
...
Sur Render, `__DIR__` pointe vers `/opt/render/project/src/`, donc le chemin final sera :
- `/opt/render/project/src/arduino-data` ✅
...
2. Vérifiez que le **Mount Path** est exactement : `/opt/render/project/src/arduino-data`
