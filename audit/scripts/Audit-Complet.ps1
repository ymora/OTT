...
# Note: Phase 10 utilise maintenant le module Checks-Performance
    ...
    # NOUVEAU: VÃƒÂ©rifier optimisations .filter() sans useMemo
    ...
    if ($filterOptimizationIssues.Count -gt 0) {
        ...
        $auditResults.Warnings += "Performance: $($filterOptimizationIssues.Count) fichier(s) avec .filter() non optimisÃƒÂ©s"
        $auditResults.Scores["Performance"] = [Math]::Max(7, $auditResults.Scores["Performance"] - 0.5)
    } else {
        Write-OK "  Optimisations .filter() appropriÃƒÂ©es"
    }
    
    # NOUVEAU: VÃƒÂ©rifier variables inutilisÃƒÂ©es
    Write-Info "Analyse variables inutilisÃƒÂ©es..."
    $unusedVariables = @()
    foreach ($file in $pagesFiles) {
        $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
        if ($content) {
            # Extraire les dÃƒÂ©clarations de variables (const, let, var)
            ...
            foreach ($decl in $varDeclarations) {
                ...
                # Ignorer les hooks React et les variables systÃƒÂ¨me
                if ($varName -notmatch '^(use|set|is|has|can|should|will|did|prev|next|current|ref)$') {
                    # Compter les occurrences (dÃƒÂ©claration + utilisations)
                    $usageCount = ([regex]::Matches($content, "\b$([regex]::Escape($varName))\b")).Count
                    # Si utilisÃƒÂ© seulement 1 fois (dÃƒÂ©claration), c'est inutilisÃƒÂ©
                    if ($usageCount -eq 1) {
                        $line = ($content.Substring(0, $decl.Index) -split "`n").Count
                        # Extraire le chemin complet du fichier
                        $filePath = $file.FullName.Replace((Get-Location).Path + "\", "").Replace("\", "/")
                        $unusedVariables += "${filePath}:${line}:${varName}"
                    }
                }
            }
        }
    ...
    if ($unusedVariables.Count -gt 0) {
        Write-Warn "  $($unusedVariables.Count) variable(s) possiblement inutilisÃƒÂ©e(s)"
        Write-Info "  Format: fichier:ligne:variable"
        foreach ($var in $unusedVariables | Select-Object -First 20) {
            Write-Info "    - $var"
        }
        if ($unusedVariables.Count -gt 20) {
            Write-Info "    ... et $($unusedVariables.Count - 20) autre(s)"
        }
        # Ajouter les dÃƒÂ©tails dans les warnings avec nom et emplacement (format structurÃƒÂ© pour IA)
        $varDetails = ($unusedVariables | Select-Object -First 20) -join " | "
        $auditResults.Warnings += "Code mort: $($unusedVariables.Count) variable(s) inutilisÃƒÂ©e(s) dÃƒÂ©tectÃƒÂ©e(s) - DÃƒÂ©tails: $varDetails"
        $auditResults.Scores["Performance"] = [Math]::Max(7, $auditResults.Scores["Performance"] - 0.3)
    } else {
        Write-OK "  Aucune variable inutilisÃƒÂ©e dÃƒÂ©tectÃƒÂ©e"
    }
    
    # NOUVEAU: VÃƒÂ©rifier doublons de code (fonctions dupliquÃƒÂ©es)
    # Ã¢Å¡Â Ã¯Â¸Â IMPORTANT: Ignorer les noms gÃƒÂ©nÃƒÂ©riques React qui sont normaux (handleSubmit, handleChange, etc.)
    Write-Info "Analyse doublons de code..."
    $duplicateFunctions = @()
    $allFunctionNames = @{}
    
    # Liste des noms de fonctions gÃƒÂ©nÃƒÂ©riques ÃƒÂ  ignorer (faux positifs normaux en React)
    $ignoredFunctionNames = @(
        # Handlers React gÃƒÂ©nÃƒÂ©riques
        ...
        'handleSelect', 'handleToggle', 'handleRefresh', 'handleUpdate', 'handleCreate',
        # Toggle functions
        ...
        # Data functions (gÃƒÂ©nÃƒÂ©riques - normaux dans diffÃƒÂ©rents composants)
        'fetchData', 'loadData', 'saveData', 'updateData', 'deleteData', 'getData', 'setData',
        'refetch', 'invalidate', 'refresh', 'reload',
        # State flags (gÃƒÂ©nÃƒÂ©riques - normaux)
        'isLoading', 'isError', 'isSuccess', 'isOpen', 'isVisible', 'isActive', 'isDisabled',
        'isConnected', 'isReady', 'isPaused', 'isStreaming',
        # React hooks
        'useState', 'useEffect', 'useCallback', 'useMemo', 'useRef', 'useReducer',
        # Noms trÃƒÂ¨s gÃƒÂ©nÃƒÂ©riques (variables/fonctions communes)
        ...
        # Utilitaires gÃƒÂ©nÃƒÂ©riques (peuvent ÃƒÂªtre dupliquÃƒÂ©s lÃƒÂ©gitimement)
        'debounce', 'throttle', 'delay', 'wait', 'sleep'
    )
    ...
    foreach ($file in $pagesFiles) {
        ...
        if ($content) {
            ...
            foreach ($func in $functions) {
                ...
                # Ignorer les noms gÃƒÂ©nÃƒÂ©riques (faux positifs)
                ...
                # Ignorer les noms qui se terminent par des suffixes gÃƒÂ©nÃƒÂ©riques (variables/fonctions communes)
                if ($funcName -match '(Data|List|Items|Array|Map|Set|Ref|State|Props|Config|Options|Params|Args|Handler|Callback|Function|Fn)$') {
                    $ignoredCount++
                    continue
                }
                
                # Ignorer les noms trÃƒÂ¨s courts (1-3 caractÃƒÂ¨res) - souvent des variables temporaires
                if ($funcName.Length -le 3) {
                    $ignoredCount++
                    continue
                }
                
                # Ignorer les fonctions qui commencent par "on" (event handlers React - normaux d'ÃƒÂªtre dupliquÃƒÂ©s)
                if ($funcName -match '^on[A-Z]') {
                    $ignoredCount++
                    continue
                }
    ...
    Write-Info "  Total fonctions analysÃƒÂ©es: $totalFunctionsFound (ignorÃƒÂ©es: $ignoredCount, uniques: $($allFunctionNames.Count))"
    if ($duplicateFunctions.Count -gt 20) {
        Write-Warn "  $($duplicateFunctions.Count) fonction(s) dupliquÃƒÂ©e(s) dÃƒÂ©tectÃƒÂ©e(s) (dÃƒÂ©tection automatique - peut contenir des faux positifs)"
        Write-Info "  Ã°Å¸â€™Â¡ Note: Certaines fonctions peuvent ÃƒÂªtre lÃƒÂ©gitimement dupliquÃƒÂ©es dans diffÃƒÂ©rents composants (handlers similaires)"
        foreach ($dup in $duplicateFunctions | Select-Object -First 5) {
            Write-Info "    - $dup"
        }
        if ($duplicateFunctions.Count -gt 5) {
            Write-Info "    ... et $($duplicateFunctions.Count - 5) autre(s)"
        }
        $auditResults.Warnings += "Code dupliquÃƒÂ©: $($duplicateFunctions.Count) fonction(s) dupliquÃƒÂ©e(s) (ÃƒÂ  vÃƒÂ©rifier manuellement)"
    } elseif ($duplicateFunctions.Count -gt 0) {
        Write-Warn "  $($duplicateFunctions.Count) fonction(s) dupliquÃƒÂ©e(s) dÃƒÂ©tectÃƒÂ©e(s)"
        foreach ($dup in $duplicateFunctions | Select-Object -First 5) {
            Write-Info "    - $dup"
        }
        $auditResults.Warnings += "Code dupliquÃƒÂ©: $($duplicateFunctions.Count) fonction(s) dupliquÃƒÂ©e(s)"
    } else {
        Write-OK "  Aucun doublon de fonction dÃƒÂ©tectÃƒÂ©"
    }
    
    # NOUVEAU: VÃƒÂ©rifier complexitÃƒÂ© par fichier
    Write-Info "Analyse complexitÃƒÂ© par fichier..."
    $complexFiles = @()
    foreach ($file in $pagesFiles) {
        $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
        if ($content) {
            ...
            # Fichier volumineux (>500 lignes) ou complexitÃƒÂ© ÃƒÂ©levÃƒÂ©e (>50 conditions)
    ...
    if ($complexFiles.Count -gt 0) {
        ...
        $auditResults.Recommendations += "Refactorisation: $($complexFiles.Count) fichier(s) volumineux/complexe(s) ÃƒÂ  considÃƒÂ©rer"
    } else {
        Write-OK "  ComplexitÃƒÂ© des fichiers acceptable"
    }
